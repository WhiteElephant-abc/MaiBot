...
def list_prompt_files():
    ...
    try:
        if not PROMPTS_DIR.exists():
            return PromptCatalogResponse(files=[])
...
def get_default_prompt_file(filename: str):
...
def list_prompt_versions(filename: str):
    """列出指定 Prompt 的自定义版本。"""
...
def get_prompt_version_file(filename: str, version_id: str):
    """读取指定 Prompt 自定义版本内容。"""
...
def activate_prompt_version(filename: str, version_id: str):
    """启用指定 Prompt 自定义版本。"""
...
def delete_prompt_version(filename: str, version_id: str):
...
def update_prompt_file(filename: str, request: PromptUpdateRequest):
...
def reset_prompt_file(filename: str):
